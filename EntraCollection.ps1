...
function Invoke-GetTokens {
	...
	if($TenantID -and $ARM -and -not $ClientSecret -and -not $ClientID){
		Get-Token-WithDeviceCode -TenantID $TenantID
	}

	if($TenantID -and $Graph -and -not $ClientSecret -and -not $ClientID){
		Get-Token-WithDeviceCode -TenantID $TenantID
	}
}
...
function Invoke-ValidUPN {
	...
	[string]$Delay
		  ...
		  function help {
			...
			if (-not $UsernameFile -and -not $DomainName -and -not $FirstName -and -not $LastName -and -not $NamesFile -and -not $OutputFilePath -and -not $StopOnFirstMatch -and -not $ConvertName -and -not $style) {
				help
				return
			}
